...
function harness({ client = {}, ref = process.env.APP_REF, initialStorage = [], storageError = false } = {}) {
  ...
  return { run: code => vm.runInContext(code,context), node, timers, logs, context, source, storage,
